...
float *vertexB;
int mode = 0;
int dim = sides * 3 + sides * 6 + sides * 3;
...
void cylinder(float radius, float height, int sides) {
	...
	vertexB = (float* ) malloc(sizeof(float)*dim * 3);
	...
	for (i = 0; i < sides; i++) {
        vertexB[j++] = 0;
        vertexB[j++] = height*0.5;
        vertexB[j++] = 0;

        vertexB[j++] = cos(i * step * M_PI / 180.0)*radius;
        vertexB[j++] = height*0.5;
        vertexB[j++] = -sin(i * step *M_PI / 180.0)*radius;

        vertexB[j++] = cos((i+1) * step * M_PI / 180.0)*radius;
        vertexB[j++] = height*0.5;
        vertexB[j++] = -sin((i + 1) * step *M_PI / 180.0)*radius;
		//glVertex3f(0, height*0.5, 0);
		//glVertex3f(cos(i * step * M_PI / 180.0)*radius, height*0.5, -sin(i * step *M_PI / 180.0)*radius);
...
// enter GLUT's main cycle
    ...
    glBindBuffer(GL_ARRAY_BUFFER, buffers[0]);
    glBufferData(GL_ARRAY_BUFFER, sizeof(float)*dim*3, vertexB, GL_STATIC_DRAW);
